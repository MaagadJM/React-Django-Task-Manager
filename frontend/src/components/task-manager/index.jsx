    ...
    return (
        <div style={styles.card}>
            ...
            <form onSubmit={addTask} style={styles.form}>
                <input style={styles.input} value={input} onChange={e => setInput(e.target.value)} placeholder="What needs to be done?" />
                <button style={styles.btn} type="submit">Add</button>
            </form>

            {error && <p style={styles.error}>{error}</p>}
            {loading ? <p style={styles.hint}>Loading…</p>
                : tasks.length === 0 ? <p style={styles.hint}>No tasks yet. Add one above!</p>
                    : (
                        <ul style={styles.list}>
                            {tasks.map(task => (
                                <li key={task.id} style={styles.item}>
                                    <input type="checkbox" checked={task.completed} onChange={() => toggleTask(task)} style={{ cursor: 'pointer', width: 18, height: 18 }} />
                                    <span style={{ ...styles.title, textDecoration: task.completed ? 'line-through' : 'none', color: task.completed ? '#94a3b8' : '#1e293b' }}>
                                        {task.title}
                                    </span>
